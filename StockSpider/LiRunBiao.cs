...
namespace StockSpider
{
    ...
    /// 获取利润表数据
    /// </summary>
    public class LiRunbiao : BaseSpider
    {
        private string baseUrl = "http://money.finance.sina.com.cn/corp/go.php/vFD_ProfitStatement/stockid/{0}/ctrl/{1}/displaytype/4.phtml";
        ...
        public override IList<BsonDocument> GetData(string area, string stockCode)
        {
            ...
            {
                Console.WriteLine("正在处理利润表: " + stockCode + "," + i);
                ...
                if (dataTable == null)
                {
                    Console.WriteLine("没有找到利润表数据," +stockCode + ":" + i);
                    continue;
                }
                ...
                for (int j = 0; j < reportCnt; j++)
                {
                    ...
                    foreach (var row in dataTable.Rows)
                    {
                        ...
                        if (mchRst.Success)
                        {
                            ...
                            if (row.Cells.Count > (j+1))
                            {
                                doc.Add(new BsonElement(strColPrefix, getCellData(row.Cells[j + 1].InnerText)));    
                            }
                        }
